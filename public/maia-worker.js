 ...
 * Adapted from CSSLab/maia-platform-frontend (GPL-3.0): paths made relative to
 * the worker's own location (so it works under the /chess-cadet/ Pages base),
 * and WASM forced single-threaded (GitHub Pages can't set COOP/COEP).
 *
 ...
 *   { type: 'status', status }            status: loading|no-cache|downloading|ready|error
 *   { type: 'progress', progress }
 *   { type: 'error', message, id? }
 *   { type: 'inference-result', id, logitsMove, logitsValue }
 */

// Resolve paths relative to THIS worker file (handles the /chess-cadet/ base).
const BASE = self.location.href.replace(/[^/]*$/, '');

importScripts(BASE + 'ort/ort.wasm.min.js');
const ORT = ort;
ORT.env.wasm.wasmPaths = BASE + 'ort/';
ORT.env.wasm.numThreads = 1; // single-threaded: no SharedArrayBuffer on Pages

// ── IndexedDB storage (so the ~44MB model downloads once, then works offline) ─
const DB_NAME = 'ChessCadetMaia';
...
self.onmessage = async (e) => {
  ...
  try {
    switch (msg.type) {
      ...
      case 'inference': {
        if (!session) {
          postMessage({ type: 'error', message: 'Model not initialized', id: msg.id });
          return;
        }
        ...
        postMessage({ type: 'inference-result', id, logitsMove: logitsMove.buffer, logitsValue: logitsValue.buffer }, [
          logitsMove.buffer,
          logitsValue.buffer,
        ]);
        break;
    ...
    postMessage({ type: 'error', message: (err && err.message) || 'Worker error', id: msg.id });
  }
};
